...
namespace PlaneSpotter.EntityFramework.Queries
{
    public class GetSightingByParaQuery : iGetSightingByParaQuery
    {
        ...
        public GetSightingByParaQuery(SightingDbContextFactory contextFactory)
        {
        ...
        public async Task<IEnumerable<Sighting>> Execute()
        {
            using (PlaneSpotterDBContext context = _contextFactory.Create())
            {
                ...
                //return sightingDTO.Where(x => x.Planemake == "").Select(x => new Sighting(x.Id, x.Planemake, x.Planemodel, x.Planeregistration, x.Location, x.DateTime, x.Photo));
            }
        }
    }
}
